...
the feature branch will be automatically be deleted.


Throughout the project we made a project board where we took the weekly assignment on 
‘README_PROJECT’ and thereafter made them into as small task as we could. And we try to do 
...
done in a okay timeframe, so now all the task are in ‘Done’. But we were now super to assign 
the task on the board. 
![Flowchart of new issue](images/project%20board.png)
...
this entire semester, he has not contributed to the project nor report at all, except for one 
co-authored commit back in the 4th of September. This problem with Radmehr was brought up to our
...
Before runing these tests, please delete the mychirp.db file in the ```\src\Chirp.Web``` 
